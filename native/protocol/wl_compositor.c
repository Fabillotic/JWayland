...
JNIEXPORT jobject JNICALL Java_dev_fabillo_jwayland_protocol_WLCompositor_create_1surface(JNIEnv *env, jobject obj) {
	...
	const struct wl_interface *inf = get_interface_by_name("wl_surface");
	if(!inf) return NULL;
...
JNIEXPORT jobject JNICALL Java_dev_fabillo_jwayland_protocol_WLCompositor_create_1region(JNIEnv *env, jobject obj) {
	jclass WLProxy_class = (*env)->FindClass(env, "dev/fabillo/jwayland/WLProxy");
	jfieldID WLProxy_native_ptr = (*env)->GetFieldID(env, WLProxy_class, "native_ptr", "J");
	jmethodID WLProxy_init = (*env)->GetMethodID(env, WLProxy_class, "<init>", "()V");

	struct wl_proxy *wproxy = (struct wl_proxy*)(intptr_t)(*env)->GetLongField(env, obj, WLProxy_native_ptr);
	const struct wl_interface *inf = get_interface_by_name("wl_region");
	if(!inf) return NULL;
...
int wl_compositor_dispatcher(const void *implementation, void *target, uint32_t opcode, const struct wl_message *msg, union wl_argument *args) {
	...
	jclass listener_class = (*env)->FindClass(env, "dev/fabillo/jwayland/protocol/WLCompositor$WLCompositorListener");
...
JNIEXPORT void JNICALL Java_dev_fabillo_jwayland_protocol_WLCompositor_addListener(JNIEnv *env, jobject obj, jobject listener) {
	jclass WLProxy_class = (*env)->FindClass(env, "dev/fabillo/jwayland/WLProxy");
	jfieldID WLProxy_native_ptr = (*env)->GetFieldID(env, WLProxy_class, "native_ptr", "J");
	jmethodID WLProxy_init = (*env)->GetMethodID(env, WLProxy_class, "<init>", "()V");
	...
	*data = env;
	wl_proxy_add_dispatcher(wproxy, wl_compositor_dispatcher, listener_ref, data);
}
